...
upstream="vq_wav2vec_kmeans wav2vec2_large_ll60k wav2vec2_xlsr hubert_large_ll60k unispeech_sat_large wavlm_base wavlm_base_plus wavlm_large"

for i in $upstream; do
    echo "Running $i"
    python run_downstream.py -d mosei -m train -n mosei-$i -u $i
done
